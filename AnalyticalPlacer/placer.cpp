...
bool Placer::spread_block() {
	...
	if (!strong_anchor && !weak_anchor) { // No anchor
		...
		calc_HPWL();
		cout << "Iteration: " << it + 1 << " HPWL: " << WL << " DISP: " << disp << ". Inner loop converged at iter " << iter << "." << endl;
		clearscreen();
		block_plot();
		it++;
		return true;
	...
	if (!update_block(Qij_pseudo, ci_pseudo, di_pseudo)) error = true;
	calc_HPWL();
	cout << "Iteration: " << it + 1 << " HPWL: " << WL << " DISP: " << disp << ". Inner loop converged at iter " << iter << "." << endl;
	sprintf(msg, "Iteration %d: Press 'Proceed' to continue. Current HPWL: %.2f. Current cell displacement: %.2f.", it + 1, WL, disp);
	update_message(msg);
	return false;
}
