...
bool Maxsat::optm() {
	...
	if (weighted) {
		...
		sprintf(msg, "[Complete] Total weight: %d. Visited nodes: %d, Run time: %.2f seconds [Created by: Weixuan Yang]", best, visited, runtime);
	}
	else {
		cout << "\n\n[Unweighted MAX-SAT]\n\nSatisfied clauses: " << best << " \nVisited nodes: " << visited << " \nRun time: " << runtime << " seconds" << endl;
		sprintf(msg, "[Complete] Satisfied clauses: %d. Visited nodes: %d, Run time: %.2f seconds [Created by: Weixuan Yang]", best, visited, runtime);
	}
	update_message(msg);
...
void plot_init(void) {
	...
	init_graphics("MAX-SAT Solver", WHITE);
